# libby websearch Design Specification
...
**Tech Stack:** aiohttp (async HTTP), asyncio.gather (parallel execution), scholarly (Google Scholar), typer (CLI), rich (progress/table display).

---

## Overview

`libby websearch` searches multiple academic databases in parallel, merges results by DOI, and outputs BibTeX or JSON format.

### Key Features

1. **Multi-source parallel search**: Crossref + Semantic Scholar + Scholarly (Google Scholar) in parallel
2. **Serpapi separate execution**: Controlled API usage (max 5 pages), no confirmation needed
3. **Unified filter translation**: `SearchFilter` converts user parameters to native API filters
4. **DOI fallback**: DOI input automatically triggers `fetch → extract` workflow
5. **Field merging dedup**: Merge fields from multiple sources for same DOI
...
libby/
├── models/
│   ├── search_result.py    # SearchResult, SearchResults, SerpapiExtraInfo
│   └── search_filter.py    # SearchFilter unified filter model
├── api/
│   ├── crossref.py         # Extend: search() method
│   ├── semantic_scholar.py # Extend: search() method
│   ├── scholarly.py        # New: Google Scholar scholarly package wrapper
│   └── serpapi.py          # Extend: search() method
├── core/
│   └── websearch.py        # WebSearcher orchestrator
├── cli/
│   └── websearch.py        # CLI command
│   └── __main__.py         # Register websearch command
```

### Data Flow

```
User Input (query/DOI)
    │
    ├─[is_doi?]─► fetch workflow ─► extract workflow
    │
    └─[else]──► websearch workflow
                    │
                    ├─► Build SearchFilter
                    │
                    ├─► Parallel: Crossref + S2 + Scholarly
                    │       │
                    │       └─► Merge by DOI
                    │
                    ├─► Serpapi (separate, max 5 pages)
                    │       │
                    │       └─► Merge + Extra Info
                    │
                    └─► Output: BibTeX/JSON + serpapi_extra.json
```
...
|------|-----------|-------|
| 1 | SearchFilter model | `models/search_filter.py` + tests |
| 2 | SearchResult model | `models/search_result.py` + tests |
| 3 | Crossref search extension | `api/crossref.py` + tests |
| 4 | Semantic Scholar search extension | `api/semantic_scholar.py` + tests |
| 5 | Scholarly wrapper | `api/scholarly.py` + tests |
| 6 | Serpapi search extension | `api/serpapi.py` + tests |
| 7 | WebSearcher core | `core/websearch.py` + tests |
| 8 | CLI websearch command | `cli/websearch.py` + tests |
| 9 | CLI registration | `cli/__main__.py` modification |
...
- Test: `tests/models/test_search_filter.py`

- [ ] **Step 1: Create SearchFilter dataclass**
...
- Test: `tests/models/test_search_result.py`

- [ ] **Step 1: Create SearchResult dataclasses**
...
class SearchResult:
    ...
    def merge_from(self, other: "SearchResult") -> "SearchResult":
        ...
        elif not self.year and other.year:
...
class SearchResults:
    ...
    def to_bibtex(self, citekey_config) -> str:
        ...
        Uses existing citekey logic from extract/fetch.
        """
        ...
        for r in self.results:
            if r.doi:  # Only output entries with DOI
                ...
                metadata = BibTeXMetadata(
                    citekey=citekey_gen.generate(r),
...
from libby.models.search_result import SearchResult, SerpapiExtraInfo, SearchResults
...
def test_search_result_merge():
    ...
    r2 = SearchResult(
        doi="10.1234/test",
        title="Test Paper: A Longer Title",  # Longer title
        journal="Nature",
...
Read `libby/api/crossref.py` to find insertion point.

- [ ] **Step 2: Add search() method**

```python
# libby/api/crossref.py (add import and method)
...
```python
# tests/api/test_crossref.py (add tests)
...
- Test: `tests/api/test_semantic_scholar.py` (add tests)
...
```python
# libby/api/semantic_scholar.py (add import and method)
...
```python
# tests/api/test_semantic_scholar.py (add tests)
...
@pytest.mark.asyncio
async def test_search_with_venue():
...
- Test: `tests/api/test_scholarly.py`

**Note:** Requires adding `scholarly` to dependencies in `pyproject.toml`.

- [ ] **Step 1: Add scholarly dependency**
...
- [ ] **Step 2: Create Scholarly wrapper**
    ...
    async def search(
        ...
        def _sync_search():
            ...
            try:
                ...
                # Handle anti-bot or network errors
                pass
            return results
...
async def test_search_with_year_filter():
    """Test year filter adds keywords."""
    api = ScholarlyAPI()
...
```bash
git add libby/api/scholarly.py tests/api/test_scholarly.py pyproject.toml
...
- Test: `tests/api/test_serpapi.py` (add tests)
...
```python
# libby/api/serpapi.py (add method)
...
```python
# tests/api/test_serpapi.py (add tests)
...
- Test: `tests/core/test_websearch.py`

- [ ] **Step 1: Create WebSearcher**
...
class WebSearcher:
    ...
    async def search_single_source(
        ...
        """Search single source only."""
        # Implementation for --source parameter
        ...
    ...
    def _parse_crossref(self, item: dict) -> SearchResult:
        ...
        return SearchResult(
            ...
            abstract=None,  # Crossref usually doesn't have abstract
...
@pytest.mark.asyncio
async def test_search_parallel(searcher):
...
@pytest.mark.asyncio
async def test_serpapi_quota_handling(searcher):
    """Test Serpapi quota detection."""
    if not searcher.serpapi:
        pytest.skip("No Serpapi key")
    
    with patch.object(searcher.serpapi, 'search', new_callable=AsyncMock) as mock:
        mock.return_value = ([], True)  # Quota reached
        
        results = await searcher.search("test", limit=10)
...
- Test: `tests/cli/test_websearch.py`
...
def websearch(
    ...
    source: Optional[str] = typer.Option(None, "--source", help="Single source"),
    ...
    Examples:
        ...
        libby websearch 10.1234/test  # DOI
    ...
    async def run():
        ...
        with Progress(...) as progress:
            ...
            if source:
                results = await searcher.search_single_source(
                    query, source.lower(), filter, limit, no_serpapi
                )
            else:
...
```bash
git add libby/cli/websearch.py tests/cli/test_websearch.py
...
**Files:**
- Modify: `libby/cli/__main__.py`

- [ ] **Step 1: Register websearch command**

```python
# libby/__main__.py (add after fetch registration)
...
```bash
git add libby/__main__.py
git commit -m "feat: register websearch CLI command
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_websearch_with_filter():
    """Test search with year filter."""
    config = LibbyConfig()
    searcher = WebSearcher(config)
    ...
    # All results should be from 2023 or later
...
- [ ] `README.md` - Add websearch usage examples
- [ ] `CHANGELOG.md` - Document new feature
- [ ] `CLAUDE.md` - Update websearch section with actual behavior
...
| Internal consistency | ✅ Models match usage |
| Scope check | ✅ Single implementation scope |
| Ambiguity check | ✅ All behaviors defined |

---

## Execution Handoff

Spec saved to `docs/superpowers/specs/2026-04-12-libby-websearch-design.md`.
